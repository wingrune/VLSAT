...
def Parser(add_help=True):
    ...
    parser.add_argument('--scan_name', type=str, default='_vh_clean_2.labels.ply', help='what is the name of the output filename of the ply generated by your segmentation method.')
    ...
    parser.add_argument('--min_seg_size', type=int,default=0,help='Minimum number of points of a segment.')
...
def process(pth_scan, scan_id, label_type, verbose=False) -> list:
    ...
    pth_pd  = os.path.join(define.SCANNET_DATA_PATH,scan_id,scan_id+args.scan_name)
    ...
    pth_ins_segm = os.path.join(define.SCANNET_INS_SEGM_PATH, scan_id+define.SCANNET_INS_SEGM_SUFFIX)
    
    cloud_gt, points_gt, labels_gt, segments_gt = util.load_scannet(pth_ply, pth_agg, pth_seg)
    scene_data = torch.load(pth_ins_segm)
    
    
    if args.verbose: print('filtering input segments.. (ori num of segments:',len(scene_data),')', filter_segment_size)
    segments_pd_filtered=list()
    for i, obj in enumerate(scene_data):
        pts = points_gt[obj['segments'],:]
        #print(seg_id, pts.shape)
        if len(pts) > filter_segment_size:
            segments_pd_filtered.append(i)
    ...
    for seg_id in segment_ids:
        segs_neighbors[int(seg_id)] = [int(i) for i in segment_ids]
    ...
    for seg_id in segment_ids:
        size_segments_gt[seg_id] = len(scene_data[seg_id]['segments'])
        instance2labelName[seg_id] = scene_data[seg_id]['label']
    ...
    ''' Find best corresponding segment ''' # we use gt
    ...
    # We use GT segmentation here, therefore we use 1 to 1 segmentation
    print("Number of object", len(segment_ids))
    for segment_id in segment_ids:
        name = instance2labelName[segment_id]
        if name == '-' or name == 'none':
            segment_id_filtered = -1
            continue
        map_segment_pd_2_gt[segment_id] = segment_id
        if segment_id not in gt_segments_2_pd_segments:
            gt_segments_2_pd_segments[segment_id] = list()
        gt_segments_2_pd_segments[segment_id].append(segment_id)
...
def gen_relationship(split:int, map_segment_pd_2_gt:dict,instance2labelName:dict,gt_segments_2_pd_segments:dict,
    ...
    for i, seg_i in map_segment_pd_2_gt.items():
        for j, seg_j in map_segment_pd_2_gt.items():
            if i == j:
                continue
            split_relationships.append([int(i),int(j), idx_in_txt_new, name_same_segment])
            split_relationships.append([int(j),int(i), idx_in_txt_new, name_same_segment])
...
if __name__ == '__main__':
    ...
    for scan_id in tqdm(sorted(scan_ids)):
        ...
        pth_ins_segm = os.path.join(define.SCANNET_INS_SEGM_PATH, scan_id+define.SCANNET_INS_SEGM_SUFFIX)
        if not os.path.exists(pth_ins_segm):
             continue
