...
def process(pth_scan, scan_id, label_type, verbose=False) -> list:
    ...
    #if args.verbose: print('filtering input segments.. (ori num of segments:',len(os.listdir(pth_ins_segm)),')', filter_segment_size)
    segments_pd_filtered=list()
    instance2labelName = dict()
    size_segments_gt = dict()
    for filename in os.listdir(pth_ins_segm):
        ...
        if len(pts) > filter_segment_size:
            segments_pd_filtered.append(idx)
            size_segments_gt[idx] = len(pred_mask["pred_mask"])
            instance2labelName[idx] = filename.split("_")[-1].split(".")[0]
