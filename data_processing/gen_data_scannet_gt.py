...
def process(pth_scan, scan_id, label_type, verbose=False) -> list:
    ...
    if args.verbose: print('filtering input segments.. (ori num of segments:',len(segment_ids),')', filter_segment_size)
    segments_pd_filtered=list()
    for seg_id in segment_ids:
        pts = points_gt[np.where(segments_gt ==seg_id)]
    ...
    for seg_id in uni_seg_gt_ids:
        ...
        if len(uq_label) > 1:
            ...
            if len(uq_label) > 1:
                ...
                max_value = len(labels[labels==uq_label[0]])
                max_id = uq_label[0]
        ...
        if uq_label[-1] == 0:
            ...
            name = util_label.NYU40_Label_Names[uq_label[-1]-1]
            # print(name)
    ...
    print(np.where(segments_gt == 20)[0].shape)
    exit()
    if verbose:
        print('instance2labelNames:')
        print(instance2labelName)
